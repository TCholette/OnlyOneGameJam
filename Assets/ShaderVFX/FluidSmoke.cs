...
//Comments to be added later WIP
public class FluidSmoke : MonoBehaviour
{
    ...
    private void UpdateSmokeTexture()
    {
        ...
        for (int i = 0; i < GridSize; i++)
        {
            for (int j = 0; j < GridSize; j++)
            {
                ...
                colors[j * GridSize + i] = new Color(densityValue, densityValue, densityValue);
            }
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 emissionWorldPos = transform.position + new Vector3(
            (_emissionPoint.x - GridSize * 0.5f) * CellSize,
            (_emissionPoint.y - GridSize * 0.5f) * CellSize,
            0f
        );
